        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-5'>
            {
                data.map((price, index) => <form onSubmit={handleSubmit(onSubmit)} className='card p-5 w-full' key={price._id}>
                    <input className='mt-3 text-2xl class="input border w-full p-2  w-full' defaultValue={price.type} type="text" />
                    <input className='mt-3 text-2xl class="input border w-full p-2 w-full' defaultValue={price.price}type="text" />
                    {
                        price.services.map((service , index) => <input key={index} {...register(`service${index}`, { required: true , value: service })}
                            className='mt-3 class="input border w-full max-w-lg p-2'  type="text" />
                        )
